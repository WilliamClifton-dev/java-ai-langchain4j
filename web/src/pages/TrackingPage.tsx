    ...
    <div className="tracking-form-grid">
      ...
      <form className="entry-form" onSubmit={(event) => { event.preventDefault(); trainingMutation.mutate(training); }}>
        ...
        <div className="compact-form-grid">
          ...
          <label className="field" htmlFor="training-intensity"><span>强度</span><select id="training-intensity" name="intensity" value={training.intensity} onChange={(event) => { setTraining((current) => ({ ...current, intensity: event.target.value as TrainingInput['intensity'] })); trainingKey.current = undefined; }}><option value="LOW">低</option><option value="MODERATE">中等</option><option value="HIGH">高</option></select></label>
        </div>

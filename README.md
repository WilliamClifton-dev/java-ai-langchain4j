...
HBTI Coach 是一个基于 Spring Boot 与 LangChain4j 的个性化体重管理助手后端。当前版本已经建立第一条可运行链路：HTTP 请求经过场景路由组合对应 Prompt，再调用带 MySQL 会话记忆的 HBTI Agent。
...
尚未实现：用户认证与资源归属、HBTI 服务端评分、计划引擎、每日记录、周报工具、RAG、流式输出和生产限流。当前版本是后端基础纵向切片，不是可公开运营的完整产品。
...
当前除普通对话和规则解释外，计划、打卡和周报场景尚未接入确定性业务工具，因此不能把模型生成内容视为已保存的业务数据。
